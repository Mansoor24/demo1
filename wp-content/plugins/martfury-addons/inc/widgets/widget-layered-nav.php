	...
	class Martfury_Widget_Layered_Nav extends WC_Widget {
		...
		protected function layered_nav_list( $terms, $taxonomy, $query_type, $height, $search, $attribute_type ) {
			...
			if ( $attribute_type == 'default' ) {
				...
				} elseif(class_exists( 'WCBoost\VariationSwatches\Swatches' )) {
			...
			$style = $height ? 'style="max-height:' . intval( $height ) . 'px"' : '';

			echo '<ul class="woocommerce-widget-layered-nav-list ' . esc_attr( $list_class ) . '" data-height="' . esc_attr( $height ) . '"' . $style. '>';
